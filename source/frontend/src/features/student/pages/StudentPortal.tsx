          ...
          if (r.items && r.items.length > 0) {
            r.items.forEach((item: any) => {
              items.push({
                ...
                requestId: r.id,
                assetName: item.asset?.name || (item.toolset ? `${item.toolset.name} (x${item.quantity})` : 'Thiết bị không tên'),
                serialNumber: item.asset?.serialNumber || item.toolset?.code || 'N/A',
                type: item.asset?.type || (item.toolset ? 'Bộ dụng cụ' : 'N/A'),
